...
namespace SweetSugar.Scripts.GUI.Boost
{
    ...
    public class BoostIcon : MonoBehaviour
    {
        ...
        private void Awake()
        {
            checkIcon = transform.Find("checkIcon")?.gameObject;
            wheel = transform.Find("white_wheel")?.gameObject;
            Lock = transform.Find("Lock")?.gameObject;
            Indicator = transform.Find("Indicator")?.gameObject;
            BoostShop = MenuReference.THIS.BoostShop.gameObject.GetComponent<BoostShop>();
            if (check != null) return;
            if (Resources.Load("Prefabs/Check") != null)
            {
        ...
        public void ActivateBoost()
        {
            ...
            {
                ActivateSelectScreenAndBuyBooster();
            }
        ...
        public void Check(bool checkIt)
        {
            switch (type)
            {
        ...
        public void ActivateSelectScreenAndBuyBooster()
        {
            MenuReference.THIS.ActionsSelectScreen.SetActive(true);
            ActionsManager.Instance.endEvent = () =>
            {
                InitScript.Instance.BuyBoost(type, 0, 1);
                // ActivateBoost();
                ActionsManager.Instance.Close();
